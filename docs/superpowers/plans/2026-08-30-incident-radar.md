...
- **실행 중 조정 — Phase 순서 재배치(2026-09-02):** T14 완료 후, **T15 + Phase 7(17~21)
  ...
  - 동기: 백엔드 연속 작업 뒤 T16→프론트 6태스크를 몰아 하며 전체 그림을 다시 잡는
    편이 이해·진행에 낫다는 판단.
- **실행 중 조정 — CORS 를 T20 에서 당김(2026-09-02):** Phase 7 을 뒤로 미뤘더니 T23
  대시보드를 브라우저에서 열 때 `CORS_ORIGIN` 미설정으로 전 API 가 차단됨. CORS 한 줄
  (`app.enableCors({ origin: CORS_ORIGIN })` + env 스키마에 `CORS_ORIGIN` 추가, dev 기본
  `http://localhost:3001`)만 T23 과 함께 선반영. helmet·throttler·`.env.example` 감사·
  콤마 목록/헤더 세부는 T20 에 그대로 남김.
- **실행 중 조정 — T23 p95 타일 보류(2026-09-02):** T23 스탯 타일 4번째("수집→알림 p95")는
  코어에 지연 데이터 소스가 없음(백엔드는 로그로만 남기고, 지연 조회 엔드포인트는 스트레치
  #2). 4칸 레이아웃은 유지하되 4번 타일은 "이후 편에서" 자리표시자로 두고, 실제 데이터
  배선은 스트레치 #2(성능 리포트) 착수 시. 나머지 3타일(최근 N분 에러 / 24h 알림·실패 /
  cooldown 중 서비스)은 실데이터로 채움.
